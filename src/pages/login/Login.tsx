...
interface LoginProps {
  dialogMode?: boolean;
  onClose?: () => void;
  skipRedirect?: boolean; // ⭐ Prop mới
}

const Login: React.FC<LoginProps> = ({ 
  dialogMode = false, 
  onClose,
  skipRedirect = false // ⭐ Default false
}) => {
      ...
      { email, otp },
      {
        onSuccess: async (res: any) => {
          ...
          if (token) {
            ...
            await queryClient.invalidateQueries({ queryKey: ["login-info"] }); // ⭐ Thêm dòng này
            ...
            // ⭐ Chỉ redirect khi KHÔNG trong booking flow
            if (!skipRedirect) {
              // Navigate về home như bình thường
              window.location.href = "/home";
            }
            // Nếu skipRedirect = true, không làm gì cả
            // BookingConfirmTab sẽ tự xử lý redirect đến /payments
          } else {
